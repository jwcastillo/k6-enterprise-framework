#!/usr/bin/env bash
# build-binary-standalone.sh — Compile standalone client repo into a self-contained binary
#
# Adapted from build-binary.sh for standalone repos exported via export-client.sh.
# The client name is auto-detected from export-manifest.json — no --client flag needed.
#
# The binary embeds all compiled JS bundles using Go's //go:embed.
# No external files are needed at runtime.
#
# Usage:
#   ./bin/build-binary-standalone.sh [--platform linux/amd64] [--verify]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
EMBED_DIR="${ROOT_DIR}/framework/cmd/k6-embedded"
...
print_help() {
  cat <<EOF
${BOLD}k6 Standalone Client — build-binary-standalone.sh${RESET}

Compile this standalone client's load testing solution into a self-contained binary.
All JS bundles are embedded in the binary — no external files needed at runtime.

The client name is auto-detected from export-manifest.json.

USAGE:
  ./bin/build-binary-standalone.sh [OPTIONS]
...
EXAMPLES:
  ./bin/build-binary-standalone.sh
  ./bin/build-binary-standalone.sh --platform linux/amd64 --verify
  ./bin/build-binary-standalone.sh --platform darwin/arm64
...
# ── Auto-detect client from export-manifest.json ─────────────────────────────
if [[ ! -f "${ROOT_DIR}/export-manifest.json" ]]; then
  log_error "export-manifest.json not found in project root."
  log_error "This script is intended for standalone repos exported via export-client.sh."
  exit 1
fi

CLIENT=$(node -e "console.log(JSON.parse(require('fs').readFileSync('${ROOT_DIR}/export-manifest.json','utf8')).client)" 2>/dev/null)

if [[ -z "${CLIENT}" ]]; then
  log_error "Could not read 'client' field from export-manifest.json"
  exit 1
fi

# Validate client name: alphanumerics, hyphens, underscores only
if [[ ! "${CLIENT}" =~ ^[a-zA-Z0-9_-]+$ ]]; then
  log_error "Invalid client name '${CLIENT}' in export-manifest.json. Only alphanumerics, hyphens and underscores are allowed."
  exit 1
fi

log_info "Auto-detected client: ${CLIENT}"
...
if [[ ! -f "${CLIENT_DIR}/config/default.json" ]]; then
  log_error "Missing required config/default.json"
  exit 1
fi

if [[ ! -d "${EMBED_DIR}" ]]; then
  log_error "Framework embed directory not found: framework/cmd/k6-embedded"
  log_error "Ensure the framework was exported correctly."
  exit 1
fi
...
echo -e "${BOLD}"
echo "  ╔══════════════════════════════════════════════╗"
echo "  ║   k6 Standalone — Binary Compilation         ║"
echo "  ╚══════════════════════════════════════════════╝"
echo -e "${RESET}"
...
# In standalone repos, webpack builds to dist/ directly
DIST_DIR="${ROOT_DIR}/dist"

if [[ ! -d "${DIST_DIR}" ]]; then
  log_error "Expected dist directory not found: dist/"
  log_error "Check that scenarios/ has at least one .ts file."
  exit 1
fi

SCRIPT_COUNT=$(find "${DIST_DIR}" -name "*.js" -not -path "*/binaries/*" | wc -l | tr -d ' ')
if [[ "${SCRIPT_COUNT}" -eq 0 ]]; then
  log_error "No compiled JS found in dist/"
  exit 1
fi
log_success "TypeScript bundle compiled (${SCRIPT_COUNT} scripts in dist/)"
...
# Clean previous scripts (not the .keep sentinel)
...
# Exclude the binaries/ directory which is the build output location
find "${DIST_DIR}" -name "*.js" -not -path "*/binaries/*" -print0 | while IFS= read -r -d '' jsfile; do
  # Compute relative path from DIST_DIR
  relpath="${jsfile#${DIST_DIR}/}"
  target_dir="${SCRIPTS_DIR}/$(dirname "${relpath}")"
  mkdir -p "${target_dir}"
  cp "${jsfile}" "${target_dir}/"
done

EMBEDDED_COUNT=$(find "${SCRIPTS_DIR}" -name "*.js" | wc -l | tr -d ' ')
log_success "Staged ${EMBEDDED_COUNT} scripts into framework/cmd/k6-embedded/scripts/"

# Copy data files for open() support at runtime
CLIENT_DATA_DIR="${ROOT_DIR}/data"
DATA_FILE_COUNT=0
if [[ -d "${CLIENT_DATA_DIR}" ]]; then
  cp -r "${CLIENT_DATA_DIR}/." "${DATA_DIR}/"
  DATA_FILE_COUNT=$(find "${DATA_DIR}" -type f ! -name ".keep" | wc -l | tr -d ' ')
  log_success "Staged ${DATA_FILE_COUNT} data files into framework/cmd/k6-embedded/data/"
else
  log_info "No data/ directory found — skipping"
fi
