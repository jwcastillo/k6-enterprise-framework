...
Todos los directorios `clients/<name>/scenarios/` siguen la misma taxonomía de 5 buckets (TST-01 / Fase 2):

| Bucket | Propósito | Ejemplo | Ejecutar |
|--------|-----------|---------|----------|
| `api/` | Smoke probes por endpoint | `api/smoke-users.ts` | `--client=_reference --scenario=api/smoke-users` |
| `flow/` | Flujos integrados multi-paso | `flow/auth-flow.ts` | `--client=_reference --scenario=flow/auth-flow` |
| `domain/` | Escenarios a nivel de servicio, sub-agrupados por servicio (`domain/<servicio>/<acción>.ts`) | `domain/orders-lifecycle.ts` | `--client=_reference --scenario=domain/orders-lifecycle` |
| `chaos/` | FCI / inyección de fallos / resiliencia | `chaos/fci-spike.ts` | `--client=_reference --scenario=chaos/fci-spike` |
| `perf/` | Capacidad, breakpoint, stress, soak | `perf/breakpoint-tier.ts` | `--client=_reference --scenario=perf/breakpoint-tier` |

**No son buckets:** `unit/` vive bajo `test/` (Vitest, Node.js), no en `clients/*/scenarios/`. No se permite un 6º bucket sin enmendar esta taxonomía.

Este cliente `_reference/` es el layout canónico — replicar su estructura al iniciar un nuevo cliente.
...
| 2 | `flow/auth-flow` | Intermedio | HTTP | Retry, ContractValidation | < 1000ms |
| 3 | `flow/checkout-flow` | Avanzado | HTTP | Pagination, WeightedSwitch, PerformanceHelper | < 800ms |
| 4 | `api/16-redis-data-pool` | Intermedio | HTTP+Redis | SharedArray, DataPool, Teardown | < 500ms |
| 5 | `domain/orders-lifecycle` | Básico | k6-builtins (hermético) | Counter, Trend, Checks | n/a |
| 6 | `chaos/fci-spike` | Básico | k6-builtins (hermético) | Rate, Counter, lógica de ventana | n/a |
| 7 | `perf/breakpoint-tier` | Básico | k6-builtins (hermético) | Trend tags, mapeo por tier | thresholds por tier |
...
Cada llamada a `bin/run-test.sh` REQUIERE dos flags y NO son intercambiables:

- `--client=<nombre>` — selecciona qué directorio `clients/<nombre>/` usar.
- `--scenario=<bucket>/<path>` — la ruta DEBE empezar con uno de los 5 buckets canónicos (`api`, `flow`, `domain`, `chaos`, `perf`).

### 1. `invalid bucket 'xyz'`

```bash
# Incorrecto — 'bc' no es un bucket canónico
./bin/run-test.sh --client=mi-cliente --scenario=bc/orders/get-order --profile=smoke
# [ERROR] invalid bucket 'bc' in --scenario='bc/orders/get-order'

# Correcto — usar un bucket canónico como prefijo
./bin/run-test.sh --client=mi-cliente --scenario=domain/orders/get-order --profile=smoke
```

### 2. `--client=` faltante

```bash
# Incorrecto — sin flag --client
...
Consultar la tabla de buckets al inicio de este README para la taxonomía completa. Tooling: solo pnpm — nunca npm / yarn / npx.
